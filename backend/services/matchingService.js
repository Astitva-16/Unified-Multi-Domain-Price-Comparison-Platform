const jaccardSimilarity = (str1, str2) => {
  const set1 = new Set(str1.split(" ").filter(Boolean));
  const set2 = new Set(str2.split(" ").filter(Boolean));

  let intersection = 0;

  set1.forEach((item) => {
    if (set2.has(item)) {
      intersection++;
    }
  });

  const union = set1.size + set2.size - intersection;

  if (union === 0) return 0;

  return (intersection / union) * 100;
};


const isPriceProximate = (
  price1,
  price2,
  tolerance = 0.2
) => {
  if (price1 === 0 || price2 === 0) {
    return true;
  }

  const difference =
    Math.abs(price1 - price2) /
    Math.max(price1, price2);

  return difference <= tolerance;
};


const matchProduct = (product1, product2) => {

  const name1 =
    product1.normalized_name || "";

  const name2 =
    product2.normalized_name || "";


  const nameSimilarity =
    jaccardSimilarity(name1, name2);


  /*
   * IMPORTANT:
   *
   * If normalized names are exactly same,
   * they are the same product.
   *
   * We don't care if mock prices are
   * different.
   */

  const exactNameMatch =
    name1 === name2 &&
    name1.length > 0;


  const priceProximate =
    isPriceProximate(
      product1.price,
      product2.price
    );


  const priceScore =
    priceProximate ? 100 : 0;


  const ratingDiff =
    Math.abs(
      product1.rating -
      product2.rating
    );


  const ratingSimilarity =
    Math.max(
      0,
      100 - ratingDiff * 10
    );


  /*
   * Exact name = automatic match
   */

  if (exactNameMatch) {

    return {
      product1_id: product1.id,
      product2_id: product2.id,

      product1_name: product1.name,
      product2_name: product2.name,

      product1_platform:
        product1.platform_name,

      product2_platform:
        product2.platform_name,

      name_similarity: "100.0",

      price_proximity: priceScore,

      confidence: "100.0",

      is_match: true,
    };
  }


  /*
   * Otherwise use normal scoring
   */

  const confidence =
    nameSimilarity * 0.6 +
    priceScore * 0.3 +
    ratingSimilarity * 0.1;


  return {
    product1_id: product1.id,
    product2_id: product2.id,

    product1_name: product1.name,
    product2_name: product2.name,

    product1_platform:
      product1.platform_name,

    product2_platform:
      product2.platform_name,

    name_similarity:
      nameSimilarity.toFixed(1),

    price_proximity:
      priceScore,

    confidence:
      confidence.toFixed(1),

    is_match:
      confidence > 70,
  };
};


const groupSimilarProducts = (products) => {

  const groups = [];

  const used = new Set();


  for (
    let i = 0;
    i < products.length;
    i++
  ) {

    if (used.has(i)) {
      continue;
    }


    const group = [
      products[i]
    ];

    used.add(i);


    for (
      let j = i + 1;
      j < products.length;
      j++
    ) {

      if (used.has(j)) {
        continue;
      }


      const match =
        matchProduct(
          products[i],
          products[j]
        );


      if (match.is_match) {

        group.push(
          products[j]
        );

        used.add(j);
      }
    }


    groups.push(group);
  }


  return groups;
};


const formatProductGroups = (groups) => {

  return groups.map((group) => {

    /*
     * Don't mutate original group
     */

    const sortedByRating =
      [...group].sort(
        (a, b) =>
          b.rating - a.rating
      );


    const sortedByPrice =
      [...group].sort(
        (a, b) =>
          a.price - b.price
      );


    const bestProduct =
      sortedByRating[0];

    const cheapest =
      sortedByPrice[0];

    const mostExpensive =
      sortedByPrice[
        sortedByPrice.length - 1
      ];


    return {

      id:
        bestProduct.id ||
        bestProduct.normalized_name,

      product_name:
        bestProduct.name,

      normalized_name:
        bestProduct.normalized_name,

      image:
        bestProduct.image || "",

      category:
        bestProduct.category ||
        "shopping",

      variants:
        group,

      best_rating:
        Math.max(
          ...group.map(
            (p) => p.rating
          )
        ),

      price_range: {

        min:
          cheapest.price,

        max:
          mostExpensive.price,

        difference:
          mostExpensive.price -
          cheapest.price,

        best_deal:
          cheapest.platform_name,
      },

      platforms_count:
        group.length,

      average_rating:
        (
          group.reduce(
            (sum, p) =>
              sum + p.rating,
            0
          ) / group.length
        ).toFixed(1),
    };
  });
};


module.exports = {
  jaccardSimilarity,
  isPriceProximate,
  matchProduct,
  groupSimilarProducts,
  formatProductGroups,
};